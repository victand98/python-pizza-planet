...
PRICE_RANGES = {"beverage": (0.25, 10), "ingredient": (0.25, 5), "size": (5, 40)}
...
size_names = ["Small", "Medium", "Large", "Extra Large", "Party Size"]
...
def get_random_ingredients(max_ingredients: int):
    ...
    while len(selected_ingredients) < num_ingredients:
        ingredient = ingredients[fake.random_int(min=0, max=len(ingredients) - 1)][
            "_id"
        ]
...
def get_random_beverages(max_beverages: int):
    ...
    while len(selected_beverages) < num_beverages:
        beverage = beverages[fake.random_int(min=0, max=len(beverages) - 1)]["_id"]
...
def populate_fake_orders():
    ...
    for _ in range(NUMBER_OF_ORDERS):
        ...
        elements = ElementManager.get_by_id_list(ingredients_ids + beverages_ids)
        total_price = OrderController.calculate_order_price(size_price, elements)
